...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    await message.edit(content="<:Giveaway1:1400865340841267311> **GIVEAWAY ENDED** <:Giveaway1:1400865340841267311>", embed=embed)
                    await message.reply(f"<:Giveaway1:1400865340841267311> Congrats {winner}, you won **{giveaway[5]}!**, Hosted by <@{int(giveaway[3])}>")
                    await self.cursor.execute("DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?", (message.id, message.guild.id))
                    await self.connection.commit()
    ...
    async def gstart(self, ctx,
        ...
        embed = discord.Embed(title=f"<:Giveaway1:1400865340841267311> {prize}",
                              description=f"Winner(s): **{winners}**\nReact with <:Giveaway1:1400865340841267311> to participate!\nEnds <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\nHosted by {ctx.author.mention}", color=0x000000)
        ...
        message = await ctx.send("<:Giveaway1:1400865340841267311> **GIVEAWAY** <:Giveaway1:1400865340841267311>", embed=embed)
        try:
