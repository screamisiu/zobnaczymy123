...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    await message.edit(content="<a:Giveaway:1400865710447525968> **GIVEAWAY ENDED** <a:Giveaway:1400865710447525968>", embed=embed)
                    await message.reply(f"<a:Giveaways:1400865710447525968> Congrats {winner}, you won **{giveaway[5]}!**, Hosted by <@{int(giveaway[3])}>")
                    await self.cursor.execute("DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?", (message.id, message.guild.id))
                    await self.connection.commit()
    ...
    async def gstart(self, ctx,
        ...
        embed = discord.Embed(title=f"<a:Giveaway:1400865710447525968> {prize}",
                              description=f"Winner(s): **{winners}**\nReact with <a:Giveaway:1400865710447525968> to participate!\nEnds <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\nHosted by {ctx.author.mention}", color=0x000000)
        ...
        message = await ctx.send("<a:Giveaway:1400865710447525968> **GIVEAWAY** <a:Giveaway:1400865710447525968>", embed=embed)
        try:
